...
def read_from_container(container: str, key: str):
    cmd = f"docker exec {container} curl -s http://localhost:8081/kv/{key}"
    try:
...
def check_data_consistency(num_keys: int):
    ...
    for key_id in range(num_keys):
        ...
        # Check followers
        for follower in ["follower1", "follower2", "follower3", "follower4", "follower5"]:
...
def plot_results(results_by_quorum: dict) -> None:
    ...
    try:
        ...
        # Force line and marker colors to red
        plt.plot(
            quorums,
            avg_latencies,
            marker="o",
        ...
        # Ensure labels and title use red text explicitly
        plt.xlabel("Write Quorum Size", fontsize=12, color='white')
        plt.ylabel("Latency (ms)", fontsize=12, color='red')
        plt.title("Write Latency vs Quorum Size (10K writes, 20 threads)", fontsize=14, color='white')
        plt.legend()
        # Make grid lines subtle but red
        plt.grid(True, color='red', alpha=0.1)
        plt.xticks(quorums)

        # Force tick labels to red
        ax = plt.gca()
        ax.tick_params(axis='x', colors='white')
        ax.tick_params(axis='y', colors='white')

        plt.tight_layout()
        plt.savefig("quorum_vs_latency.png", dpi=300, facecolor='white')
